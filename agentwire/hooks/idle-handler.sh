#!/bin/bash
# AgentWire session-end observer — Hermes ``on_session_end`` lifecycle hook.
#
...
#   1. If a scheduled task context exists (``~/.agentwire/tasks/<session>.json``),
...
DEBUG_LOG="/tmp/agentwire-session-end-debug.log"
...
${summary_content}"
    ...
    (
        out=$($AGENTWIRE notify-parent --queued -q "session finished" 2>&1) \
