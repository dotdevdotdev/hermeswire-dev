"""CLI for diagnostics — ``hermeswire doctor`` and ``hermeswire network``.
...
def cmd_network_status(args) -> int:
    ...
    # Print header
    print("HermesWire Network Status")
    print("=" * 60)
    ...
    if result.returncode == 0:
        sessions = [s for s in result.stdout.strip().split("\n") if s and not s.startswith("hermeswire")]
        if sessions:
        ...
        for i, issue in enumerate(issues, 1):
            if issue["type"] == "machine_unreachable":
                ...
                if issue["service"] == "portal":
                    print("       hermeswire portal start")
                elif issue["service"] == "tts":
                    print("       hermeswire tts start")
                print("       hermeswire tunnels check  # Verify tunnel health")
                print()
                ...
                print()
                print("     To fix:")
                print("       hermeswire tunnels up")
                print()

        print("-" * 60)
        print()
        print("Run: hermeswire doctor    # Auto-fix common issues")
...
def _render_skill_section() -> int:
    ...
    for name, state in sorted(skill_drift().items()):
        ...
        if state == "ok":
            ...
            print("     Run: hermeswire hooks install")
            issues += 1
        else:
...
def _render_damage_control_section() -> int:
    ...
    # Kill switch — ``enabled`` in ~/.hermeswire/damagecontrol.yml gates ALL
    ...
    if not safety_enabled:
        ...
        print("       Fix: set enabled: true in ~/.hermeswire/damagecontrol.yml")
        issues += 1
    else:
    ...
    try:
        ...
        if state in prov.REFUSING_STATES:
            ...
            print("  [..] No canonical hermeswire install found (bootstrap/dev checkout)")
    ...
    if set(by_state) - {"ok"}:
        ...
        if by_state.get("newer"):
            ...
            print("       Fix: git pull --ff-only && hermeswire rebuild")
        if set(by_state) - {"ok", "newer"}:
            print("       Fix: hermeswire safety install --yes")
        issues += 1
    else:
    ...
    for label, drift, source_getter, live_dir in (
        ...
        if unknown:
            ...
            if weakened:
                ...
                print("       Fix: hermeswire safety install --yes --force (keeps a backup)")
                issues += 1
            if extended:
                print(
    ...
    try:
        ...
        if inert:
            ...
            print("       ~/.hermeswire/tooldefs/ missing the stable `id:` fields).")
            issues += 1
        else:
...
def _find_unmigrated_task_projects() -> list[str]:
    """Local projects with an inline ``.hermeswire.yml`` ``tasks:`` block but no
    promoted ``.hermeswire.tasks.yml`` (#736).

    This is the concrete #720/#721 regression: the task-split relocated where
    tasks are READ from (``.hermeswire.tasks.yml``) without migrating existing
    ...
    for proj in projects_mod.get_projects("local"):
        ...
        cfg_file = proj_dir / ".hermeswire.yml"
        try:
...
def _render_task_migration_section() -> int:
    ...
    for name in unmigrated:
        print(
            f"  [!!] Project {name}: tasks defined in .hermeswire.yml but never "
            "migrated to .hermeswire.tasks.yml — they will NOT run under "
            "ensure/scheduler."
        )
    print(
        "     Run `hermeswire tasks migrate` then `hermeswire tasks promote` "
...
def _find_dead_managed_shims() -> list[dict]:
    ...
    inside a live ``hermeswire-kokoro``/``hermeswire-stt`` session — say/transcribe
    ...
    if getattr(getattr(cfg, "tts", None), "backend", "default") == "default":
        checks.append((
            ...
            "hermeswire kokoro start", "say falls back to browser voice",
        ))
...
def find_orphaned_worktrees(rows: list[dict]) -> list[dict]:
    ...
    ``hermeswire spawn --branch`` used to guarantee by never registering at
...
def _render_orphaned_worktrees_section() -> int:
    ...
    print("       Review with `hermeswire worktree --list --all`, then tear down with "
          "`hermeswire worktree --remove <name> -p <repo>` (merged-branch guards apply).")
    return len(orphans)
...
def _render_orphaned_history_section() -> int:
    ...
    if not orphaned:
        ...
        print("       Recover with `hermeswire history migrate` — the transcripts are "
              "intact, just keyed to the old path. Until then `hermeswire restart` on a "
...
def _render_pending_messages_section() -> int:
    ...
    if parked:
        print(f"       Parked recipient(s): {', '.join(parked)} — these deliver on "
              "their own once the usage limit resets (`hermeswire limits status`), "
              "so this is FYI, not a failure.")
    print("       Inspect: `hermeswire msg inbox -s <session>`   "
          "Force: `hermeswire msg flush -s <session> --force`")
    return 1
...
def _newest_installed_source_mtime(pkg_dir: Path | None = None) -> float:
    """Newest mtime among the installed ``hermeswire`` package's ``.py`` files."""
    if pkg_dir is None:
...
def _render_scheduler_staleness_section() -> int:
    ...
    ``hermeswire scheduler serve`` is a long-running Python process — like the
    MCP server, it imports its modules once at start and never re-reads them.
    ``hermeswire rebuild`` updates the on-disk package but can't touch an
    ...
    if started_at is None:
        if _scheduler_daemon_predates_pid_stamp():
            ...
            print("       Liveness and the staleness check below can't be verified "
                  "until it restarts.")
            print("       Fix: hermeswire scheduler stop && hermeswire scheduler start")
            return 1
    ...
    print(f"  [!!] Scheduler daemon has been running {age_days:.1f}d — "
          "predates the most recent `hermeswire rebuild`")
    ...
    for b in blocked:
        ...
        if b["status"] == "detector_error":
            ...
            print("       Grep the sweep's record: "
                  "grep detect_failed ~/.hermeswire/prompt-router-events.jsonl")
            print(f"       hermeswire output -s '{b['session']}'   # read it yourself")
            continue
        print(f"  [!!] {where}: blocked on a {b['kind']} prompt for {waited} "
              f"— {b['status']}")
        print(f"       {b['question']}" + (f"  ({b['summary']})" if b["summary"] else ""))
        if b["status"] == "unrouted":
            print("       Nobody has been notified. Fix: hermeswire limits install "
            ...
            print(f"       Routed to {b['parent']}, still unanswered. Answer it "
                  "yourself:")
        print(f"       hermeswire output -s '{b['session']}'   # inspect first")
        print(f"       hermeswire prompts answer -s '{b['session']}' "
...
def _render_mcp_import_section() -> int:
    ...
    """
    proc = subprocess.run(
        [sys.executable, "-c", "import hermeswire.mcp_server"],
    ...
    print("  [!!] MCP server entrypoint fails to import — every mcp__hermeswire__* "
...
@dataclass
class _SecretPath:
    """An hermeswire path that must never be readable by anyone but its owner."""
    path: Path
...
def _render_secrets_permissions_section(
    *, auto_confirm: bool = False, dry_run: bool = False,
    ...
    ``chmod 600 ~/.hermeswire/.env`` was documented in two places and enforced
    ...
    automatic, unlike the forced ``chmod`` on hermeswire's OWN writes: tightening
...
def _render_custom_services_section() -> int:
    ...
    for svc in registry:
        ...
        if healthy:
            ...
            print(f"     Run: hermeswire services up {svc.name}")
            issues += 1
        ...
        if risk:
            ...
            print("     Fix: move the secret to ~/.hermeswire/.env and read it "
    ...
    for name in sorted(BETA_FLAG_NAMES):
        if name not in enabled:
            print(f"  [..] beta.{name}: off — set 'beta.{name}: true' in "
                  "~/.hermeswire/config.yaml to enable")
            continue
        if name == "voice_layer":
            if os.environ.get(API_KEY_ENV, "").strip():
                ...
                print(f"     Fix: add {API_KEY_ENV} to ~/.hermeswire/.env (chmod 600)")
                issues += 1
        else:
...
def cmd_doctor(args) -> int:
    ...
    if tmux_path:
        ...
        for opt, why in (
            ...
            if val == "on":
                ...
                print("     Recommended config: hermeswire init (tmux step), or see")
    ...
    # 3. Check HermesWire scripts
    print("\nChecking HermesWire scripts...")

    say_path = shutil.which("say")
    if say_path:
        print(f"  [ok] say: {say_path}")
    else:
        print("  [..] say: not found (optional, use 'hermeswire say' directly)")

    # 4. Check hermeswire-owned hook files (existence AND drift from packaged source)
    print("\nChecking HermesWire hooks...")

    hook_meta = {
        "hermeswire-permission.sh": ("Permission hook", False,
    ...
    print("\nChecking HermesWire global skills...")
    ...
    if src_root is None:
        ...
        if err:
            ...
            print(f"       {src_root}")
            print("       Fix: git pull --ff-only  (then hermeswire rebuild)")
            issues_found += 1
        else:
    ...
    # `hermeswire config set` allowlist must never contain execution-plane
    ...
    if not _cfg_violations:
        ...
        print("     Fix: remove the key from EDITABLE_KEYS in hermeswire/config_cli.py")
                    ...
                    if auto_confirm or _confirm(f"     Start {service_name}?"):
                        ...
                        if service_name == "portal":
                            ...
                            if tmux_session_exists(session_name):
                                ...
                                subprocess.run(
                                    ["tmux", "send-keys", "-t", session_name, "hermeswire portal serve", "Enter"],
                                    capture_output=True,
                                )
                                print("[ok] started")
                                issues_fixed += 1
                            ...
                            if tmux_session_exists(session_name):
                                ...
                                subprocess.run(
                                    ["tmux", "send-keys", "-t", session_name, "hermeswire tts serve", "Enter"],
                                    capture_output=True,
                                )
                                print("[ok] started")
                                issues_fixed += 1
    ...
    # ~/.hermeswire/.env (docs/wiki/security/secrets.md), loaded on every
    ...
    if expected_keys:
        print("\nChecking secrets (~/.hermeswire/.env)...")
        for feature, candidates in expected_keys:
            ...
            if found:
                ...
                print(f"       Fix: add {candidates[0]}=... to ~/.hermeswire/.env")
                issues_found += 1
    ...
    if not remote_machines:
        ...
        for machine_id, machine in remote_machines.items():
            ...
            # Check if hermeswire is installed
            try:
                result = subprocess.run(
                    ["ssh", "-o", "ConnectTimeout=5", "-o", "BatchMode=yes", target, "hermeswire --version"],
                    capture_output=True,
                    text=True,
                    timeout=7,
                )
                if result.returncode == 0:
                    ...
                    print(f"    [ok] hermeswire installed ({version})")
                else:
                    print("    [!!] hermeswire not installed")
                    print(f"         Fix: ssh {target} 'pip install hermeswire-dev'")
                    issues_found += 1
                    continue
            ...
            try:
                result = subprocess.run(
                    ["ssh", "-o", "ConnectTimeout=5", "-o", "BatchMode=yes", target, "cat ~/.hermeswire/portal_url"],
                    capture_output=True,
                    text=True,
                    timeout=7,
                )
                if result.returncode == 0:
                    ...
                    print(f"         Fix: ssh {target} 'echo \"https://localhost:8765\" > ~/.hermeswire/portal_url'")
                    issues_found += 1
            except (subprocess.TimeoutExpired, subprocess.CalledProcessError):
                print("    [!!] portal_url not set")
                print(f"         Fix: ssh {target} 'echo \"https://localhost:8765\" > ~/.hermeswire/portal_url'")
    ...
    try:
        ...
        if not dangling:
            ...
            print("       Assign a parent (hermeswire msg send / --created-by) or merge/close the PR yourself.")
    except Exception as e:
    ...
    # worktrees `hermeswire spawn --branch` used to create invisibly.
    ...
    # 12. Projects whose inline .hermeswire.yml tasks were never migrated to the
    # promoted .hermeswire.tasks.yml (#736). The #720/#721 task-split moved where
    ...
    # between `hermeswire new` reporting success and the pane's `cd`) drops to
    # a bare shell the idle-reaper never touches. `hermeswire limits tick`
    ...
    try:
        ...
        if not zombie_sessions:
            ...
            print("       `hermeswire limits tick` reaps these automatically — "
                  "run it now, or check the usage-limit watchdog is installed "
                  "(`hermeswire limits install`).")
    except Exception as e:
