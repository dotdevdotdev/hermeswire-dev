"""Shared stateless helpers for the HermesWire CLI.
...
def run_hermeswire_cmd(
    args: list[str],
    json_output: bool = True,
    timeout: int = 30,
) -> dict:
    """Run hermeswire CLI command and return result.
    ...
    try:
        ...
        return {"success": False, "error": "hermeswire command not found"}
...
class AgentCommand:
    ...
    by HermesWire. For a resume launch ``--resume <id>`` continues the SAME
...
_UNATTENDED_ENV_KEYS = ("HERMESWIRE_UNATTENDED", "HERMESWIRE_UNATTENDED_ALLOW")
...
def _with_unattended_env(env: dict[str, str]) -> dict[str, str]:
    ...
    it seeds ``HERMESWIRE_UNATTENDED[=1]`` (and any per-task
    ...
    spawn, #715; this env is not rooted). For ``HERMESWIRE_UNATTENDED`` that is
    defense in depth: it TIGHTENS, so inheriting it can only ever block more.
    ``HERMESWIRE_UNATTENDED_ALLOW`` inherits on the same path and LOOSENS, which
    ...
    No leak into interactive sessions: a human's ``hermeswire new`` has no such
...
def _set_session_name_env(agent: "AgentCommand", session_name: str, created_by: str | None = None) -> None:
    """Stamp ``HERMESWIRE_SESSION_NAME`` (and, when there's a real parent,
    ``HERMESWIRE_CREATED_BY``) onto an ``AgentCommand.env``.

    Every session created via ``cmd_new`` / ``cmd_spawn`` / ``cmd_recreate``
    / ``cmd_fork`` / scheduler-spawn paths gets ``HERMESWIRE_SESSION_NAME`` so
    downstream tooling (notably the worker damage-control rules in
    ``safety/_core.py``) can identify which hermeswire session the running
    tool is part of.

    ``created_by`` of ``''`` (root/orchestrator) or ``None`` means no
    parent — ``HERMESWIRE_CREATED_BY`` is deliberately left UNSET rather than
    ...
    """
    agent.env["HERMESWIRE_SESSION_NAME"] = session_name
    if created_by:
        agent.env["HERMESWIRE_CREATED_BY"] = created_by
...
# Owner-only, matching the posture `~/.hermeswire/.env` is DOCUMENTED to have —
# and, since #887, the posture hermeswire actually enforces on every file it
...
def write_owner_only(path: Path, text: str) -> None:
    ...
    tightening the whole of ``~/.hermeswire`` is a decision for the operator,
    which is what ``hermeswire doctor`` (reporting) and ``doctor --yes``
    (healing) are for.
    """
...
def check_python_version() -> bool:
    ...
    if current_version < min_version:
        ...
        print(f"   HermesWire requires Python {min_version[0]}.{min_version[1]} or higher")
        print()
...
def check_pip_environment() -> bool:
    ...
    if marker.exists():
        ...
        print("  uv tool install hermeswire-dev")
        print("  # or: pipx install hermeswire-dev")
        print()
        print("Alternative - a dedicated venv:")
        print("  python3 -m venv ~/.hermeswire-venv")
        print("  source ~/.hermeswire-venv/bin/activate")
        print("  pip install hermeswire-dev")
        print()
        return False
...
def resolve_default_created_by(caller: str | None, target_path: Path) -> str | None:
    ...
    A service session (hermeswire-portal, -tts, -stt, -kokoro, ...) never
    ...
    ``caller`` right back to "hermeswire-portal" — this was the root cause of
...
def load_config() -> dict:
    """Load configuration from ~/.hermeswire/config.yaml."""
...
def get_source_dir() -> Path:
    """Get the hermeswire source directory from config.

    Precedence: HERMESWIRE_SOURCE_DIR env var, then dev.source_dir from
    config.yaml, then ~/projects/hermeswire-dev. The path is not validated —
    use find_source_checkout() when the caller needs a real checkout.
    """
    env_dir = os.environ.get("HERMESWIRE_SOURCE_DIR")
    if env_dir:
        return Path(env_dir).expanduser()
    config = load_config()
    source_dir = config.get("dev", {}).get("source_dir", "~/projects/hermeswire-dev")
    return Path(source_dir).expanduser()


# Where a git clone of hermeswire-dev conventionally lives. Checked in order
...
def find_source_checkout() -> Path | None:
    """Locate an hermeswire-dev source checkout, or None on a package-only install.
...
def get_portal_session_name() -> str:
    ...
    config = load_config()
    return config.get("services", {}).get("portal", {}).get("session_name", "hermeswire-portal")


def get_tts_session_name() -> str:
    """Get TTS tmux session name from config."""
    config = load_config()
    return config.get("services", {}).get("tts", {}).get("session_name", "hermeswire-tts")


def get_stt_session_name() -> str:
    """Get STT tmux session name from config."""
    config = load_config()
    return config.get("services", {}).get("stt", {}).get("session_name", "hermeswire-stt")


def get_kokoro_session_name() -> str:
    """Get default-tier Kokoro TTS shim tmux session name from config."""
    config = load_config()
    return config.get("services", {}).get("kokoro", {}).get("session_name", "hermeswire-kokoro")
...
def session_metadata_path(session_name: str) -> Path:
    ...
    and the unlink by ``hermeswire kill``. The docstring used to claim "both
    ...
    ``hermeswire kill`` — so ``../../../evil`` would have addressed, and
...
def recorded_sessions() -> list[str]:
    ...
    slashes by design (``project/branch`` is what every ``hermeswire worktree``
...
def record_session_launch(
    ...
    *,
    ...
    metadata.update({
        ...
        # them means `hermeswire restart` silently drops the third and hands the
    ...
    try:
        ...
        print(
            ...
            "recoverable from disk — `hermeswire history resume`, prompt "
...
def _display_parent(session_name: str, path: str = "") -> "str | None":
    ...
    nest based on whether the parent is actually in the list):
      1. Creator recorded at `hermeswire new` time (session metadata).
      2. `.hermeswire.yml` `parent:` field (from the session's path).
...
def _guarded_launch_command(path_str: str, agent_cmd: str | None) -> str:
    ...
    - A real recorded parent (``$HERMESWIRE_CREATED_BY`` set) gets the crash
    ...
    - No parent (root/orchestrator sessions, the genuine scheduler-dispatch
      ...
      ``$HERMESWIRE_UNATTENDED=1`` (see ``_with_unattended_env``).
    """
    quoted_path = shlex.quote(path_str)
    session_ref = "${HERMESWIRE_SESSION_NAME:-unknown session}"
    missing_body = f"cd failed at launch: {path_str}"
    notify_parent = (
        '[ -n "$HERMESWIRE_CREATED_BY" ] && hermeswire msg send --to "$HERMESWIRE_CREATED_BY" '
        f'--kind escalation --subject "hermeswire: worktree missing at launch — {session_ref}" '
        f'--body "{missing_body}" >/dev/null 2>&1'
    )
    notify_owner = (
        '[ -z "$HERMESWIRE_CREATED_BY" ] && [ "$HERMESWIRE_UNATTENDED" = "1" ] && hermeswire email '
        f'--subject "hermeswire: worktree missing — {session_ref}" '
        f'--body "{missing_body}" >/dev/null 2>&1'
    )
    alert = (
        f"echo \"hermeswire: worktree missing at launch, aborting: {path_str}\" >&2; "
...
def _launch_tmux_session(
    ...
    That launch line rides in as ``HERMESWIRE_LAUNCH_CMD`` and is `eval`'d by
...
def _get_hermeswire_path() -> str:
    """Get the full path to the hermeswire executable.
    ...
    config = load_config()
    configured_path = config.get("executables", {}).get("hermeswire")

    if configured_path:
        return os.path.expanduser(configured_path)

    # Find hermeswire in PATH
    found = shutil.which("hermeswire")
    if found:
        return found

    # Fallback to common location
    return os.path.expanduser("~/.local/bin/hermeswire")
...
def post_desktop_notification(text: str, session: str | None = None,
    ...
    Every producer of a text toast goes through here: `hermeswire notify-user`,
...
def _start_portal_local(args, attach: bool = True) -> int:
    ...
    When attach is False (used by `hermeswire up`), the portal is started
    ...
    # No tunnel auto-spawn (#420): hermeswire owns only the local portal
    # boundary. Reaching the portal from elsewhere is bring-your-own
    # (cloudflared/tailscale/ssh -L), and `hermeswire tunnels *` remains as an
    ...
    print(f"Starting HermesWire portal ({mode}) in tmux session '{session_name}'...")
    subprocess.run([
...
def _install_global_tmux_hooks() -> None:
    ...
    install_hook(
        "session-created",
        f'run-shell -b "{hermeswire_path} notify-event session_created -s #{{session_name}} >/dev/null 2>&1 || true"'
    )
    install_hook(
        "session-closed",
        f'run-shell -b "{hermeswire_path} notify-event session_closed -s #{{hook_session_name}} >/dev/null 2>&1 || true"'
    )

    # Presence tracking hooks
    install_hook(
        "client-attached",
        f'run-shell -b "{hermeswire_path} notify-event client_attached -s #{{session_name}} >/dev/null 2>&1 || true"'
    )
    install_hook(
        "client-detached",
        f'run-shell -b "{hermeswire_path} notify-event client_detached -s #{{session_name}} >/dev/null 2>&1 || true"'
    )

    # Pane creation hook (global - catches all pane creations)
    install_hook(
        "after-split-window",
        f'run-shell -b "{hermeswire_path} notify-event pane_created -s #{{session_name}} --pane-id #{{pane_id}} >/dev/null 2>&1 || true"'
    )
    ...
    install_hook(
        "session-renamed",
        f'run-shell -b "{hermeswire_path} notify-event session_renamed -s #{{session_name}} >/dev/null 2>&1 || true"'
    )

    # Activity notification hook (fires when monitor-activity is enabled on a window)
    install_hook(
        "alert-activity",
        f'run-shell -b "{hermeswire_path} notify-event window_activity -s #{{session_name}} >/dev/null 2>&1 || true"'
    )
