"""CLI for scheduled-workload execution — ``hermeswire ensure`` plus the
``task`` and ``lock`` command groups.

``ensure`` runs a named task from ``.hermeswire.yml`` with locking, retries,
...
def _ensure_remote(args, session: str, machine_id: str, json_mode: bool) -> int:
    ...
    `hermeswire ensure` command and run it on the remote machine natively.
    ...
    cmd_parts = ["hermeswire", "ensure", "-s", session, "--task", args.task, "--json"]
    if remote_project:
...
def send_task_prompt(session: str, prompt: str) -> bool:
    ...
    else (``hermeswire send``, ``prompt_router``, ``council``, ``session_cli``,
...
def cmd_ensure(args) -> int:
    ...
    if task.unknown_keys:
        print(f"Warning: task '{task.name}' sets keys hermeswire ignores: "
...
def _dispatch_shares_dir(task) -> bool:
    ...
    other, branches mixing. That is an *accident* an interactive `hermeswire new`
...
def _is_headless_dispatch(task) -> bool:
    ...
    Only the scheduler's unattended dispatches (``HERMESWIRE_UNATTENDED=1``) of
    ...
    """
    if not os.environ.get("HERMESWIRE_UNATTENDED"):
        return False
...
def _run_ensure_task_headless(args, session, task, ctx, shell, project_path, json_mode) -> int:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        (project_path / ".hermeswire").mkdir(exist_ok=True)
...
def _run_ensure_task(args, session, task, ctx, shell, project_path, json_mode) -> int:
    ...
    * **Headless** (unattended scheduler, ``HERMESWIRE_UNATTENDED=1`` and
    ...
    * **REPL** (interactive / persistent) — the tmux session + paste model,
    ...
    for attempt in range(1, max_attempts + 1):
            ...
            if task.starting_session and task.starting_session != session:
                if tmux_session_exists(task.starting_session):
                    ...
                    fork_result = subprocess.run(
                        ["hermeswire", "fork", "-s", task.starting_session, "-t", session, "--json"],
        ...
        # Ensure .hermeswire directory exists
        (project_path / ".hermeswire").mkdir(exist_ok=True)

        # Create iterations directory for loop tasks
        if task.mode == "loop":
            (project_path / ".hermeswire" / "iterations").mkdir(exist_ok=True)
        ...
        summary_glob = f".hermeswire/task-summary-{session}-{task.name}-*.md"
...
def cmd_task_list(args) -> int:
    ...
    if not tasks:
        print(f"No tasks defined in {project_path / '.hermeswire.tasks.yml'}")
        return 0
...
def cmd_task_validate(args) -> int:
    ...
    warnings = (
        [f"ignored key(s) {', '.join(task.unknown_keys)} — hermeswire does not read these"]
...
def register_ensure_parser(subparsers) -> None:
    ...
    ensure_parser = subparsers.add_parser(
        ...
        description="Execute a task from .hermeswire.yml with locking, retries, and completion detection.",
    )
    ensure_parser.add_argument("-s", "--session", required=True, help="Target session name")
    ensure_parser.add_argument("-p", "--project", help="Project path containing .hermeswire.yml (defaults to ~/projects/{session})")
    ensure_parser.add_argument("--task", required=True, help="Task name from .hermeswire.yml")
    ...
    task_parser = subparsers.add_parser(
        ...
        description="List, show, and validate tasks defined in .hermeswire.yml.",
    )
