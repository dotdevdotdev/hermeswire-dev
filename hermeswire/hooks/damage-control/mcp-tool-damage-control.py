...
# ///
"""
HermesWire MCP Tool Damage Control
=================================

Hermes Agent ``pre_tool_call`` hook for the outbound ``mcp__hermeswire__*`` tools
...
The tools run exactly ``hermeswire email …`` / ``hermeswire quo …`` under the hood
(``run_hermeswire_cmd``), so this hook *synthesizes the equivalent command* from
...
``outbound.hermeswire-email`` / ``outbound.hermeswire-quo`` rules verbatim — same
...
(``{"tool_name": "mcp__hermeswire__email_send", "tool_input": {...}, ...}``). A
...
Implementation: the body of ``hermeswire/safety/_core.py`` is inlined below
...
"""
Shared damage-control logic for both the hermeswire CLI (`hermeswire safety check`)
and the Hermes PreToolUse hooks (bash/edit/write).
...
def _find_project_config() -> Tuple[str, List[Any]]:
    ...
    (top-level ``allowed_paths``), never the agent-writable ``.hermeswire.yml``.
    Otherwise an agent could allowlist a control-plane path in ``.hermeswire.yml``
...
def _path_context_regex(path: str) -> str:
    ...
    if is_glob_pattern(path):
        ...
        # ``*.hermeswire.yml`` never matched an absolute target
        # (``> /repo/.hermeswire.yml`` sailed past every write pattern) (#678).
...
def path_ladder_haystacks(command: str, path: str) -> List[str]:
    ...
    CONTENT — ``grep -rn "rm file deletion" ~/.hermeswire/`` was refused as a
    ...
    itself MENTIONS the guarded path stays visible (``rm "~/.hermeswire/a b"``
...
def _assign_rule_id(
    ...
    ``hermeswire doctor`` / ``safety status`` / ``safety lint`` (#916).
...
#
# When a session is marked unattended (``HERMESWIRE_UNATTENDED=1``, stamped at
...
#   1. ``HERMESWIRE_UNATTENDED_ALLOW`` — the per-task extension the scheduler
...
# regardless. Interactive bypass sessions (no ``HERMESWIRE_UNATTENDED``) are
...
# machine-wide writes the scoped entry in ``~/.hermeswire/damagecontrol.yml``.
#
# NOTE (#916 / review §2): these ids only exist if the LOADED tooldefs carry
# them. ``~/.hermeswire/tooldefs/`` wins over the bundled copy and never
# self-heals, so on a drifted install five of the six name no rule at all and
# every one of these actions is blocked unattended. ``hermeswire doctor`` now
...
DEFAULT_UNATTENDED_ALLOW = [
    ...
    "outbound.hermeswire-email",  # blanket allow, any recipient (#804)
...
def is_unattended() -> bool:
    ...
    return os.environ.get("HERMESWIRE_UNATTENDED") == "1"
...
def _env_unattended_allow() -> Any:
    """Decode ``HERMESWIRE_UNATTENDED_ALLOW`` into an ``unattended_allow`` list.
    ...
    """
    raw = os.environ.get("HERMESWIRE_UNATTENDED_ALLOW", "")
    if not raw.strip():
        return []
...
def encode_unattended_allow(entries: Any) -> str:
    """Encode an ``unattended_allow`` list for ``HERMESWIRE_UNATTENDED_ALLOW``."""
...
def unattended_grant_allows(
    ...
    measure a scope against — the MCP hook's synthesized ``hermeswire email
    ...
    if not scopes:
        ...
        return False, (
            ...
            f"so it grants nothing (check `hermeswire tasks review` for the reason)"
        )
...
def load_safety_config(
    ...
    ``.hermeswire.yml``, which the policed agent can edit:

      * Global:  ``~/.hermeswire/damagecontrol.yml``
    ...
    if global_config_path is None:
        global_config_path = Path.home() / ".hermeswire" / "damagecontrol.yml"
    try:
...
PROTECTED_CONTROL_PLANE_PATHS = [
    "~/.hermeswire/damagecontrol.yml",   # global kill switch + rule knobs
    ...
    "~/.hermeswire/hooks/damage-control/*.py",  # the hook scripts themselves
    "~/.hermeswire/damage-control/*.yaml",      # the damage-control rule files
    # Execution-plane configs: hermeswire runs strings from these files through
    ...
    "~/.hermeswire/scheduler.yaml",      # scheduler gate commands run via shell
    "~/.hermeswire/config.yaml",         # service healthcheck commands run via shell
    # Per-project task-EXECUTION config (#720): split out of ``.hermeswire.yml``,
    # which is now purely declarative (type/roles/voice/parent/worktree) and
    # agent-writable again — it carries no exec vector. ``.hermeswire.tasks.yml``
    # holds the ``tasks:`` block (pre/post/on_task_end/shell/unattended_allow),
    # which hermeswire runs the same way (confused-deputy risk above), so it
    # stays protected. Authored via propose-and-promote: an agent drafts to the
    # UNPROTECTED ``.hermeswire.tasks.proposed.yml`` staging file; a human reviews
    # (``hermeswire tasks review``) and promotes (``hermeswire tasks promote``) —
    # the agent never writes the live file. See ``hermeswire/tasks_cli.py``.
    "*.hermeswire.tasks.yml",            # project task-execution config (any dir)
]
...
def control_plane_allowlist_overlaps(
    ...
    host-side override — but a broad entry (``*/.hermeswire/*``) takes the
    ...
    """
    canonical_home = "/home/hermeswire-canonical"
...
def _protected_reason(matched_pattern: str) -> str:
    ...
    ``.hermeswire.tasks.yml`` has a real, host-side remedy (propose-and-promote,
    #720) — name it so the agent doesn't dead-end. Everything else keeps the
    generic "edit on the host" message.
    """
    if matched_pattern.endswith(".hermeswire.tasks.yml"):
        return (
            "protected task-execution control-plane file (host-owned) — draft "
            "to .hermeswire.tasks.proposed.yml instead, then ask the host to run "
            "`hermeswire tasks review` and `hermeswire tasks promote`"
        )
...
# path. ``hermeswire tasks promote`` is the first case (#720/#721 review):
# it's the host-trusted program that copies a vetted draft into the protected
# ``.hermeswire.tasks.yml``. If a policed/unattended agent could just invoke
...
# "from hermeswire.tasks_cli import cmd_tasks_promote; ..."`` bypass, since a
# raw interpreter call never matches a bash command pattern at all.

PROTECTED_COMMAND_PATTERNS: List[Tuple[str, str]] = [
    (
        r'\bhermeswire\s+tasks\s+promote\b',
        "hermeswire tasks promote writes the protected .hermeswire.tasks.yml — "
...
def check_protected_bash_command(command: str) -> Tuple[bool, str]:
    ...
    normalizes to the literal form (``hermeswire tasks prom''ote`` can't sneak
...
# including ``tmux -L hermeswire kill-server``, whose rule exists to stop an
...
def check_command(command: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # `hermeswire tasks promote` is the confused-deputy escape itself, not a
...
def check_read_path(file_path: str, config: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    calls, so a ``zeroAccessPaths`` secret (``~/.hermeswire/.env``, ``~/.ssh/id_rsa``,
    ``*.pem``) could be exfiltrated by reading it directly. Hermes fires a
...
def _resolve_rules_dir() -> Path:
    ...
    user_dir = Path(os.environ.get("HERMESWIRE_DIR", os.path.expanduser("~/.hermeswire")))
...
def _resolve_tooldefs_dir():
    """User tooldefs (~/.hermeswire/tooldefs/) win; else bundled."""
    user_dir = Path(os.environ.get("HERMESWIRE_DIR", os.path.expanduser("~/.hermeswire")))
...
def _notify_unattended_block(command: str, reason: str, rule_id: str | None) -> None:
    ...
    The hook is a PEP 723 script (pyyaml-only) and can't import the hermeswire
    ...
    try:
        subprocess.Popen(
            ["hermeswire", "safety", "notify-unattended-block",
             "--reason", reason or "",
             "--rule-id", rule_id or "",
             "--command", command[:500]],
...
def _synthesize_command(tool_name: str, tool_input: dict) -> str | None:
    """Return the ``hermeswire <verb> …`` command an outbound MCP tool runs, or None.

    Keep these in lockstep with ``email_send`` / ``quo_send`` in
    ``hermeswire/mcp_server.py`` and the ``outbound.hermeswire-*`` rules.
    """
    short = tool_name.split("__")[-1]  # mcp__hermeswire__email_send -> email_send

    if short == "email_send":
        parts = ["hermeswire", "email"]
...
def main() -> None:
    ...
    if decision == "block":
        ...
        # `command` here is SYNTHESIZED from the tool call (`hermeswire email
        ...
        if rule_id and rule_id not in grants:
            message += (
                ...
                f"'{rule_id}' to its .hermeswire.tasks.yml task `unattended_allow`."
            )
        print(json.dumps({"action": "block", "message": message}))
        sys.exit(0)
    elif decision == "ask":
        # Hermes has no Claude-style (pre-conversion) `permissionDecision`; an ask escalates to
