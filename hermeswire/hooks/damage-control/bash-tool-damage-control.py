...
# ///
"""
HermesWire Bash Tool Damage Control
==================================

Hermes Agent ``pre_tool_call`` hook for the ``terminal`` tool. Decides
allow/block/ask based on the merged rule set under
``hermeswire/hooks/damage-control/rules/*.yaml`` (or the user's
``~/.hermeswire/damage-control/*.yaml`` override) plus ask-patterns generated
...
def main() -> None:
    ...
    # (the old Claude Code's ``Bash`` no longer exists), and its payload carries
