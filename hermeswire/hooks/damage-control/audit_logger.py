#!/usr/bin/env python3
"""
HermesWire Damage Control Audit Logger
======================================
Logs all security decisions (blocked, asked, allowed) to JSONL files for analysis.

Storage: ~/.hermeswire/logs/damage-control/YYYY-MM-DD.jsonl
Format: One JSON object per line (JSONL)

Fields:
- timestamp: ISO 8601 timestamp
- session_id: HermesWire session ID (from env)
...
def get_log_dir() -> Path:
    ...
    hermeswire_dir = os.environ.get("HERMESWIRE_DIR", os.path.expanduser("~/.hermeswire"))
    log_dir = Path(hermeswire_dir) / "logs" / "damage-control"
...
def get_session_context() -> dict:
    ...
    return {
        "session_id": os.environ.get("HERMESWIRE_SESSION_ID", "unknown"),
        "agent_id": os.environ.get("HERMESWIRE_AGENT_ID", "main"),
    }
