#!/bin/bash
# HermesWire permission gate — Hermes ``pre_tool_call`` approve-escalating hook.
#
...
#
# - Unattended scheduler dispatch (``HERMESWIRE_UNATTENDED=1``): allow (exit 0).
...
if [[ "${HERMESWIRE_UNATTENDED:-0}" == "1" ]]; then
    exit 0
fi
