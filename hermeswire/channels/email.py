...
class EmailConfig:
    ...
    ~/.hermeswire/.env (docs/wiki/security/secrets.md). It is never read
    from config.yaml.
    """
...
def _render_email_template(
    ...
    env = Environment(
        loader=PackageLoader("hermeswire", "templates"),
...
def send_email(
    ...
    if not email_config.api_key:
        raise EmailConfigError(
            ...
            "Add RESEND_API_KEY=re_... to ~/.hermeswire/.env "
    ...
    if not sender:
        raise EmailConfigError(
            ...
            "Set channels.email.from_address in ~/.hermeswire/config.yaml"
        )
...
def cmd_email(args) -> int:
    ...
    subject = args.subject or "[HermesWire] Notification"
