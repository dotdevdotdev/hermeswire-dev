...
TASKS_DIR = Path.home() / ".hermeswire" / "tasks"
...
# (running `hermes`) from a daemon-python (running `hermeswire`).
_PYTHON_INTERPRETERS = {"python", "python3"}


def _pid_is_hermes_agent(pid: str) -> bool:
    """Is the process *pid* a Hermes agent (not the hermeswire daemon)?

    A Hermes agent runs `hermes chat ...` under a python interpreter; the
    scheduler/portal daemon runs `hermeswire ...`. Inspecting the cmdline is
...
def generate_summary_filename(session: str, task_name: str) -> str:
    ...
    Returns:
        Relative path like .hermeswire/task-summary-mysession-2024-01-15T07-00-00.md
    """
    timestamp = datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
    return f".hermeswire/task-summary-{session}-{task_name}-{timestamp}.md"
