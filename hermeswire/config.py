"""
HermesWire configuration management.
...
DEFAULT_WORKTREE_COPY_FILES = [".env", ".hermeswire.yml", ".hermeswire.tasks.yml"]
...
class ServerConfig:
    ...
    # token (auto-generated at ~/.hermeswire/portal.token; see SECURITY.md).
    host: str = "127.0.0.1"
    port: int = 8765
    ...
    # None = use ~/.hermeswire/portal.token (auto-generated). "" = auth
...
class WorktreesConfig:
    ...
    ``hermeswire new -s <project>/<branch>``, and ``copy_files`` seeds every
    fresh worktree (including scheduler dispatches). Standalone
    ``hermeswire worktree`` sessions use :class:`WorktreeConfig` instead
    ...
    # .hermeswire.yml/.hermeswire.tasks.yml are included because worktree-dispatched
...
class WorktreeConfig:
    ...
    Governs `hermeswire worktree <name>` — the first-class primitive for
    ...
    ``hermeswire worktree`` command and its branch↔session registry.
...
class TTSConfig:
    ...
    **subprocess** (tmux ``hermeswire-kokoro``, default ``:8102``) — the portal
    ...
    voices_dir: Path = field(default_factory=lambda: Path.home() / ".hermeswire" / "voices")
    # Session-default knobs; sent to custom shims inside `options`
    exaggeration: float = 0.5
    cfg_weight: float = 0.5
    # Capability pass-through envelope — opaque to hermeswire, verbatim to the shim
    instructions: str = ""
    ...
    def __post_init__(self):
        ...
        self.voices_dir = _expand_path(self.voices_dir) or Path.home() / ".hermeswire" / "voices"
...
class STTConfig:
    ...
    the portal-managed shim **subprocess** (tmux ``hermeswire-stt``, default
...
class MachinesConfig:
    ...
    file: Path = field(
        default_factory=lambda: Path.home() / ".hermeswire" / "machines.json"
    )
...
class UploadsConfig:
    ...
    dir: Path = field(
        default_factory=lambda: Path.home() / ".hermeswire" / "uploads"
    )
    max_size_mb: int = 10
    cleanup_days: int = 7
...
class ArtifactsConfig:
    ...
    dir: Path = field(
        default_factory=lambda: Path.home() / ".hermeswire" / "artifacts"
    )
    max_size_mb: int = 10
...
class CustomServiceConfig:
    ...
    `hermeswire up` AND on portal launch, and are watched by the portal's
    service watchdog.

    Two kinds, distinguished by whether `command` is set:

    - **agent** (no `command`) — an hermeswire session created in a project
      directory; `roles`/`posture`/`context_policy` override the project's
      .hermeswire.yml when set.
    ...
    The command kind is deliberately generic: hermeswire supervises a process,
    ...
    manual `hermeswire services down` always sticks regardless of policy).
    """

    name: str
    project: Optional[str] = None
    autostart: bool = True
    roles: Optional[str] = None  # comma-separated; overrides project .hermeswire.yml
...
@dataclass
class ReplConfig:
    """Hermeswire REPL (Textual TUI) configuration.
    ...
    to the brand defaults defined in `hermeswire/repl/textual_app.py`.

    Example `~/.hermeswire/config.yaml`:
...
class BetaConfig:
    ...
    able to install hermeswire and see no trace of it, including in the tokens
    their sessions pay for.
    """

    #: The realtime voice buddy (``hermeswire buddy``, docs/wiki/voice-layer.md).
    ...
    #: description — see :mod:`hermeswire.beta`.
...
def default_config_path() -> Path:
    """``~/.hermeswire/config.yaml`` — the one spelling of the default path."""
    return Path.home() / ".hermeswire" / "config.yaml"
...
class SchedulerConfig:
    ...
    board_file: Path = field(default_factory=lambda: Path.home() / ".hermeswire" / "scheduler.yaml")
    # Run-state lives in its OWN file — the daemon NEVER rewrites board_file
    # (which holds user-authored task definitions). See #449.
    state_file: Path = field(default_factory=lambda: Path.home() / ".hermeswire" / "scheduler-state.yaml")
    state_backups: int = 5         # rotated good copies of the state file kept for recovery
    events_file: Path = field(default_factory=lambda: Path.home() / ".hermeswire" / "scheduler-events.jsonl")
    live_state_file: Path = field(default_factory=lambda: Path.home() / ".hermeswire" / "scheduler-live.json")
...
@dataclass
class Config:
    """Root configuration for HermesWire."""
...
def _apply_env_overrides(data: dict) -> dict:
    ...
    Env vars use HERMESWIRE_ prefix with double underscore for nesting.
    Example: HERMESWIRE_SERVER__PORT=9000
    """
    prefix = "HERMESWIRE_"
    ...
    for key, value in os.environ.items():
        ...
        # Parse key: HERMESWIRE_SERVER__PORT -> ["server", "port"]
...
def _dict_to_config(data: dict) -> Config:
    ...
    tts = TTSConfig(
        ...
        voices_dir=tts_data.get("voices_dir", Path.home() / ".hermeswire" / "voices"),
    ...
    machines = MachinesConfig(
        file=machines_data.get("file", "~/.hermeswire/machines.json"),
    )

    # Uploads
    uploads_data = data.get("uploads", {})
    uploads = UploadsConfig(
        dir=uploads_data.get("dir", "~/.hermeswire/uploads"),
    ...
    artifacts = ArtifactsConfig(
        dir=artifacts_data.get("dir", "~/.hermeswire/artifacts"),
    ...
    try:
        ...
        for name, channel_cls in ChannelRegistry._channels.items():
            if channel_cls.config_class:
                ...
                # are env-only — ~/.hermeswire/.env — so a stale key in yaml
                ...
                if unknown:
                    print(
                        ...
                        f"~/.hermeswire/.env, never config.yaml "
    ...
    scheduler = SchedulerConfig(
        ...
        board_file=scheduler_data.get("board_file", "~/.hermeswire/scheduler.yaml"),
        state_file=scheduler_data.get("state_file", "~/.hermeswire/scheduler-state.yaml"),
        state_backups=scheduler_data.get("state_backups", 5),
        events_file=scheduler_data.get("events_file", "~/.hermeswire/scheduler-events.jsonl"),
        live_state_file=scheduler_data.get("live_state_file", "~/.hermeswire/scheduler-live.json"),
...
def load_config(config_path: Optional[Path] = None) -> Config:
    ...
    Args:
        config_path: Path to config file. Defaults to ~/.hermeswire/config.yaml
...
def enabled_beta_flags() -> set[str]:
    ...
    ``import hermeswire.mcp_msg`` from 4.8ms to 83ms. The gate sits on hot paths
    ...
    ``_apply_env_overrides``, so ``HERMESWIRE_BETA__VOICE_LAYER=true`` behaves
