"""CLI for Hermes Agent session history — ``hermeswire history ...``.
...
orchestrator etiquette a fresh ``hermeswire new`` would (#316).
"""
...
def cmd_history_list(args) -> int:
    ...
    if args.project:
        ...
        if config is None:
            ...
            print("Use --project <path> or run from a directory with .hermeswire.yml", file=sys.stderr)
            return 1
...
def cmd_history_resume(args) -> int:
    ...
    project's .hermeswire.yml config.
    """
    session_id = args.session_id
    ...
    # orchestrator etiquette a fresh `hermeswire new` would, instead of an empty
