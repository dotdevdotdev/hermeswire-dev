---
name: hermeswire-mcp-tools
description: Reference for the `mcp__hermeswire__*` MCP tools — session/pane management, voice/TTS, tasks/locks, channels (email + quo, outbound-only), machines/tunnels/network, history/roles/projects, scheduler, desktop UI, notifications. Use when agents inside hermeswire sessions need to pick the right MCP tool instead of shelling out to the `hermeswire` CLI.
---

# HermesWire MCP Tools

**Agents running in hermeswire sessions should use MCP tools instead of CLI commands.** The hermeswire MCP server provides tools that wrap CLI functionality. Use these instead of `Bash: hermeswire <cmd>`.

## Session Management (13 tools)

| CLI Command | MCP Tool |
|-------------|----------|
| `hermeswire list` | `sessions_list()` |
| `hermeswire list --context` | `sessions_context(session="")` — context headroom (remaining %); LOW = bloated. Observe-only (#442). |
| `hermeswire new -s name` | `session_create(name="...")` |
| `hermeswire send -s name "msg"` | `session_send(session="...", message="...")` |
| `hermeswire msg send --to name "msg"` | `msg_send(to="...", text="...", kind="note", ref="")` |
| `hermeswire msg inbox -s name` | `msg_inbox(session="...")` |
| `hermeswire msg pull -s name` | `msg_pull(session="...")` |
| `hermeswire msg flush -s name` | `msg_flush(session="...")` |
| `hermeswire msg dead -s name` | `msg_dead(session="...")` |
| `hermeswire output -s name` | `session_output(session="...")` |
| `hermeswire info -s name` | `session_info(session="...")` |
| `hermeswire kill -s name` | `session_kill(session="...")` |
| `hermeswire send-keys -s name key1 key2` | `session_send_keys(session="...", keys=["..."])` |
| `hermeswire recreate -s name` | `session_recreate(session="...")` |
| `hermeswire fork -s name -t project/branch` | `session_fork(session="...", target="...")` |
| `hermeswire fork -s name -t project/branch --commit abc` | `session_fork(session="...", target="...", commit="abc")` |
| `hermeswire wait --children` | `wait_children(timeout=300)` — block on the children YOU spawned; see below |
...
**Note:** `session_create` (via `hermeswire new`) records the calling session as the new session's creator ONLY when `project_dir` is the same project you're already running in — interactive prompts (permission/plan/AskUserQuestion) in that child then route back to you as `[PROMPT from ...]` messages. Spawning into a genuinely different project defaults to a standalone root instead of nesting under you (#715); pass `created_by=<name>` to force a specific parent regardless of project (e.g. for a closely related project). Answer routed prompts with `hermeswire prompts answer -s <session> --expect <hash> <key>` via Bash (guarded compare-and-send), NEVER with raw `session_send_keys` — a late keystroke races the portal and can type into the child's input or abort its turn.

## Pane Management (9 tools)

| CLI Command | MCP Tool |
|-------------|----------|
| `hermeswire spawn --roles worker` | `pane_spawn(roles="worker")` |
| `hermeswire send --pane 1 "msg"` | `pane_send(pane=1, message="...")` |
| `hermeswire output --pane 1` | `pane_output(pane=1)` |
| `hermeswire kill --pane 1` | `pane_kill(pane=1)` |
| `hermeswire list` (in tmux) | `panes_list()` |
| `hermeswire split -n 2` | `pane_split(count=2)` |
| `hermeswire detach --pane 1 -s target` | `pane_detach(session="src", pane=1, target="target")` |
| `hermeswire jump --pane 1` | `pane_jump(pane=1)` |
| `hermeswire resize` | `pane_resize()` |

## Voice & TTS (6 tools)

| CLI Command | MCP Tool |
|-------------|----------|
| `hermeswire say "text"` | `say(text="...", display="...")` |
| `hermeswire notify-parent "text"` | `notify_parent(text="...", session="...")` |
| `hermeswire notify-user "text"` | `notify_user(text="...", session="...", priority="normal")` |
| `hermeswire notify-event EVENT` | `notify_event(event="...", session="...")` |
| `hermeswire listen start` | `listen_start()` |
| `hermeswire listen stop` | `listen_stop()` |
| `hermeswire listen cancel` | `listen_cancel()` |
| (portal API) | `transcribe(audio_base64="...", format="webm")` |
| `hermeswire tts voices` | `voices_list()` |

## Tasks & Locks (7 tools)

| CLI Command | MCP Tool |
|-------------|----------|
| `hermeswire ensure -s x --task y` | `task_run(session="x", task="y")` |
| `hermeswire task list x` | `task_list(session="x")` |
| `hermeswire task show x/y` | `task_show(session="x", task="y")` |
| `hermeswire task validate x/y` | `task_validate(session="x", task="y")` |
| `hermeswire lock list` | `lock_list()` |
| `hermeswire lock clean` | `lock_clean()` |
| `hermeswire lock remove session` | `lock_remove(session="...")` |

## Operations (12 tools)

| CLI Command | MCP Tool |
|-------------|----------|
| `hermeswire projects list` | `projects_list()` |
| `hermeswire roles list` | `roles_list()` |
| `hermeswire roles show name` | `role_show(name="...")` |
| `hermeswire machine list` | `machines_list()` |
| `hermeswire machine add id --host h --user u` | `machine_add(machine_id="...", host="...", user="...")` |
| `hermeswire machine remove id` | `machine_remove(machine_id="...")` |
| `hermeswire history list` | `history_list()` |
| `hermeswire history show id` | `history_show(session_id="...")` |
| `hermeswire history resume id -p path` | `history_resume(session_id="...", project="...")` |
| `hermeswire handoff init --title hint` | `handoff_init(title="...")` |
| `hermeswire handoff render <bundle-dir>` | `handoff_render(bundle_dir="...", story=True)` |
| `hermeswire handoff list` | `handoff_list()` |

## Channels (outbound-only)

| CLI Command | MCP Tool |
|-------------|----------|
| `hermeswire channels list` | `channels_list()` |
| `hermeswire email --body "..." --to addr` | `email_send(body="...", to="...", attachments=["..."], plain_text=False)` |
| `hermeswire quo --body "..." --to "+1..."` | `quo_send(body="...", to="+1...")` |

## Notifications & Network (5 tools)

| CLI Command | MCP Tool |
|-------------|----------|
| `hermeswire notify-event EVENT` | `notify_event(event="...")` |
| `hermeswire tunnels up` | `tunnels_up()` |
| `hermeswire tunnels down` | `tunnels_down()` |
| `hermeswire tunnels status` | `tunnels_status()` |
| `hermeswire network status` | `network_status()` |

## Status (3 tools)

| CLI Command | MCP Tool |
|-------------|----------|
| `hermeswire portal status` | `portal_status()` |
| `hermeswire tts status` | `tts_status()` |
| `hermeswire stt status` | `stt_status()` |
...
| CLI Command | MCP Tool |
|-------------|----------|
| `hermeswire scratchpad add "text" --source s` | `scratchpad_add(text, source)` |
| `hermeswire scratchpad list` | `scratchpad_list()` |
...
restarted by the portal watchdog. Start/stop is CLI-only (`hermeswire services
up|down`) — the MCP surface is read-only introspection.

| CLI Command | MCP Tool |
|-------------|----------|
| `hermeswire services list` | `services_list()` |
| `hermeswire services status` | `services_status()` |

## Scheduler (8 tools)

| CLI Command | MCP Tool |
|-------------|----------|
| `hermeswire scheduler status` | `scheduler_status()` |
| `hermeswire scheduler board` | `scheduler_board()` |
| `hermeswire scheduler live --json` | `scheduler_live()` |
| `hermeswire scheduler events --json` | `scheduler_events(tail=20, task="")` |
| `hermeswire scheduler run task` | `scheduler_run(task="...")` |
| `hermeswire scheduler report --since 8h` | `scheduler_report(since="8h", artifact=False)` |
| `hermeswire scheduler enable task` | `scheduler_enable(task="...")` |
| `hermeswire scheduler disable task` | `scheduler_disable(task="...")` |
| `hermeswire scheduler history` | `scheduler_history(limit=20)` |
...
**Chrome tab bookkeeping (MCP):** `chrome_tab_track(tab_id, url, session)` / `chrome_tab_untrack(tab_id, session)` — hermeswire can't call `tabs_close_mcp` itself (that server runs inside the calling agent's own client), so this is pure tracking: a worktree session records a verification tab right after `tabs_create_mcp`, then closes it + untracks before finishing. `worktree_remove`/`worktree_prune --gc-merged` check this during teardown and surface any tab a session never closed, so the tearing-down agent can close it. `chrome_tab_list(session)` shows what's tracked.

**Briefing Mode** (asymmetric-verbosity orchestration): an `anchor` (terse, human-facing) fans out `correspondent` worktrees (verbose researchers) via `worktree_create(roles="correspondent")`; they file reports into `research_dir()` (`~/.hermeswire/research/<session>/`) and drop a passive `msg_send(kind="ingest", ref="<path>")` pointer; the anchor `msg_pull()`s on the human's cue and briefs with `say(text=..., display=...)` (voice + toast, different content). See wiki briefing-mode.md.
